...
vec4 effect(mediump vec4 colour, Image texture, mediump vec2 tc, mediump vec2 sc) {
    ...
    float t_anim = greyscale.g + time;
    float adjust_val = 0.1 * sin(t_anim) + 0.5;
    vec2 adj = uv - vec2(adjust_val);
    adj.x *= texture_details.b / texture_details.a;
    adj.y *= texture_details.b / texture_details.a;

    vec4 hsl = HSL(tex);
    vec4 baseHsl = hsl;
    if (greyscale.g != 0.0) {
        hsl.y = 0.02;
        hsl.z *= (1.0 - adj.x * cos(greyscale.r * 0.512));
        hsl.z *= (1.0 - adj.y * cos(greyscale.r * 0.512));
    }
    if (baseHsl.z > 0.95 || baseHsl.a == 0.0) {
        hsl.a = 0.0;
    }

    tex = mix(RGB(hsl), RGB(baseHsl), 0.0);
