...
vec4 dissolve_mask(vec4 tex, vec2 texture_coords, vec2 uv) {
    ...
    vec2 f1 = centered + 50.0 * vec2(sin(-t / 143.634), cos(-t / 99.4324));
    vec2 f2 = centered + 50.0 * vec2(cos(t / 53.1532), cos(t / 61.4532));
    vec2 f3 = centered + 50.0 * vec2(sin(-t / 87.53218), sin(-t / 49.0));
    float field = (1.0 + (
...
// main effect
vec4 effect(mediump vec4 colour, Image texture, mediump vec2 tex_coords, mediump vec2 screen_coords) {
    vec4 tex = Texel(texture, tex_coords);
    vec2 uv = (tex_coords * image_details - texture_details.xy * texture_details.ba)
              / texture_details.ba;
    ...
    tex = RGB(sat);
    return dissolve_mask(tex * colour, tex_coords, uv);
}
