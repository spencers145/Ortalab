...
// main effect
vec4 effect(mediump vec4 colour, Image texture, mediump vec2 texture_coords, mediump vec2 screen_coords) {
    vec4 tex = Texel(texture, texture_coords);
    vec2 uv = (texture_coords * image_details - texture_details.xy * texture_details.ba)
              / texture_details.ba;

    float t_anim = chameleon.g + time;
    vec4 sat = HSL(tex);
    sat.a = 1.0 - (0.6 * ease((1.0 + cos(t_anim)) / 2.0));
    if (sat.b < 0.9 && sat.b > 0.0) {
        sat.a = 0.7 - (0.6 * ease((1.0 + cos(t_anim - 1.0)) / 2.0));
    }
    if (uv.x < 0.05 || uv.x > 0.95 || uv.y < 0.04 || uv.y > 0.96) {
        sat.a = chameleon.g;
    }
    if (tex.a == 0.0) sat.a = 0.0;
